...
class StatsCog(commands.Cog):
    ...
    @app_commands.describe(pool="The pool", category="Filter: NM, HD, HR or DT")
    ...
    async def leaderboard(self, interaction: discord.Interaction, pool: str, category: str = None):
        ...
        if not rows:
            return await interaction.followup.send(f"No scores yet in **{pool_row['name']}**.")
        ...
        if not maps:
            return await interaction.followup.send("No scores for this filter.")

        embed = discord.Embed(
            title=f"🏆 {pool_row['name']}{'  —  ' + category.upper() if category else ''}",
            color=0xFF66AA
        )
        embed.set_footer(text="NF scores • best score per player per map")
        ...
        for beatmap_id, meta in sorted(map_meta.items(), key=lambda x: (x[1]["mod_category"], x[1]["slot"])):
            ...
            if len(embed.fields) >= 24:
                embed.set_footer(text="⚠️ Too many maps — use category filter")
                break

        await interaction.followup.send(embed=embed)

    @app_commands.command(name="map_leaderboard", description="Leaderboard van 1 specifieke map")
    @app_commands.describe(pool="The pool", slot="Slot e.g. NM1, HD2")
    @app_commands.autocomplete(pool=pool_autocomplete)
    async def map_leaderboard(self, interaction: discord.Interaction, pool: str, slot: str):
        ...
        if not target:
            return await interaction.followup.send(f"❌ Slot `{slot}` not found.")
        ...
        if not entries:
            embed.description = "No scores yet op deze map."
        else:
            lines = [
            ...
            embed.description = "\n".join(lines)
        embed.set_footer(
            text=f"Required mods: NF + {target['mod_category']}"
            if target["mod_category"] != "NM" else "Required mods: NF"
        )
        await interaction.followup.send(embed=embed)

    @app_commands.command(name="rankings", description="Global LAN ranking — totale score")
    async def rankings(self, interaction: discord.Interaction):
        ...
        if not rows:
            return await interaction.followup.send("No pool scores yet.")
    ...
    async def lan_recap(self, interaction: discord.Interaction):
        ...
        if all_stats:
            ...
            embed.add_field(
                name="🏅 Awards",
                value=(
                    f"✨ **FC King**: {top_fc[0]} ({top_fc[1]['fc_count']} FCs)\n"
        ...
        if no_scores:
            embed.add_field(name="😴 No pool scores", value=", ".join(no_scores), inline=False)
    ...
    async def pool_recap(self, interaction: discord.Interaction, pool: str):
        ...
        embed.add_field(name="Maps played", value=str(len(maps_done)), inline=True)
        ...
        if unplayed:
            lines.append(f"\n_Not played: {', '.join(unplayed)}_")

        embed.add_field(name="📋 Map winners", value="\n".join(lines) or "—", inline=False)
        await interaction.followup.send(embed=embed)

    @app_commands.command(name="stat_rankings", description="Rankings on een specifieke stat")
    @app_commands.choices(stat=[
        app_commands.Choice(name="Accuracy",      value="accuracy"),
        app_commands.Choice(name="FC count",      value="fc"),
        app_commands.Choice(name="Maps played", value="maps"),
    ...
    async def stat_rankings(self, interaction: discord.Interaction, stat: str):
        ...
        stat_map = {
            "accuracy": ("avg_accuracy",  "Avg. Accuracy",  fmt_acc),
            "fc":       ("fc_count",      "FC Count",       lambda x: f"{x} FCs"),
            "maps":     ("total_scores",  "Total scores",  str),
        ...
        embed = discord.Embed(title=f"📊 Rankings — {label}", description="\n".join(lines) or "No data.", color=0x66AAFF)
        await interaction.followup.send(embed=embed)


    @app_commands.command(name="all_scores", description="All pool scores van iedereen, gesorteerd op score")
    ...
    async def avg_leaderboard(self, interaction: discord.Interaction):
        ...
        lines = [
            ...
            f"{r['maps_played']} maps • {r['fc_count']} FCs"
            for i, r in enumerate(rows)
        ]
        embed = discord.Embed(
            title="📊 Average Pool Score",
            description="\n".join(lines),
            color=0x66AAFF
        )
        await interaction.followup.send(embed=embed)
    ...
    async def lan_start(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            f"✅ LAN started at **{now.strftime('%d-%m-%Y %H:%M')} UTC**. Stats will be tracked from now on.",
            ephemeral=False
        )

    @app_commands.command(name="lan_stats", description="Statistieken over de LAN-sessie")
    @app_commands.describe(member="Leave empty for global stats, or pick a player")
    async def lan_stats(self, interaction: discord.Interaction, member: discord.Member = None):
        ...
        if member:
            ...
            if not pdata:
                return await interaction.followup.send(f"No data found for **{player['osu_username']}**.")
            ...
            embed.add_field(name="Scores set", value=str(pdata["total_scores"] or 0), inline=True)
            embed.add_field(name="Pool scores", value=str(pdata["pool_scores"] or 0), inline=True)
            embed.add_field(name="FCs", value=str(pdata["fc_count"] or 0), inline=True)
            embed.add_field(name="Avg. accuracy", value=fmt_acc(pdata["avg_accuracy"]) if pdata["avg_accuracy"] else "—", inline=True)
            ...
            embed = discord.Embed(title="📊 LAN Stats — Global", color=0xFFAA00)
            if since:
            ...
            if global_stats:
                embed.add_field(name="Scores set", value=str(global_stats["total_scores"] or 0), inline=True)
                embed.add_field(name="Pool scores", value=str(global_stats["pool_scores"] or 0), inline=True)
                embed.add_field(name="FCs set", value=str(global_stats["fc_count"] or 0), inline=True)
                embed.add_field(name="Avg. accuracy", value=fmt_acc(global_stats["avg_accuracy"]) if global_stats["avg_accuracy"] else "—", inline=True)
                embed.add_field(name="Active players", value=str(global_stats["active_players"] or 0), inline=True)
            ...
            if per_player:
                ...
                for p in per_player:
                    ...
                    lines.append(
                        ...
                        f"{p['fc_count']} FCs • {fmt_acc(p['avg_accuracy']) if p['avg_accuracy'] else '—'}"
                    )
                if lines:
                    embed.add_field(name="👥 Per player", value="\n".join(lines), inline=False)
